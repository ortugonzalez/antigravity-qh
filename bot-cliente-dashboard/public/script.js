document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderMain = () => {
        ...
        // V17 Stats Update Handler
        const updateStatsPanel = () => {
             const now = new Date();
             const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
             
             const ticketsHoy = allReports.filter(r => r.createdAt && r.createdAt >= startOfDay);
             document.getElementById('stat-hoy').textContent = ticketsHoy.length;
             ...
             document.getElementById('stat-empresas').innerHTML = sortedEmps.length ? sortedEmps.map((e,i) => `<span><b>${i+1}.</b> ${e[0]} <span style="opacity:0.5">(${e[1]})</span></span>`).join('') : '<span style="opacity:0.5">Sin datos</span>';
             document.getElementById('stat-servicios').innerHTML = sortedCats.length ? sortedCats.map((c,i) => `<span><b>${i+1}.</b> ${c[0]} <span style="opacity:0.5">(${c[1]})</span></span>`).join('') : '<span style="opacity:0.5">Sin datos</span>';
        };
        updateStatsPanel();
        ...
        const createItemNode = (req) => {
            ...
            if (estadoActual.toLowerCase() === 'pendiente' && req.createdAt) {
                 ...
                 if (minutesPassed < 15) {
                     ...
                     borderColor = '#00b4d8';
                     SLA_Html = `<span class="badge" style="background:#00b4d8; color:#fff; border: 1px solid rgba(255,255,255,0.3); font-size: 0.8rem; padding: 5px 10px;">⏱️ < 15 Min</span>`;
                 } else if (minutesPassed <= 30) {
                     itemClasses += ' sla-yellow';
                     borderColor = '#ffcc00';
                     SLA_Html = `<span class="badge" style="background:#ffcc00; color:#000; border: 1px solid rgba(0,0,0,0.3); font-size: 0.8rem; padding: 5px 10px;">⚠️ < 30 Min</span>`;
                 } else {
                     itemClasses += ' sla-red';
                     borderColor = '#ff3366';
                     SLA_Html = `<span class="badge" style="background:#ff3366; color:#fff; border: 1px solid rgba(255,255,255,0.4); font-size: 0.8rem; padding: 5px 10px; font-weight: 900;">🔥 > 30 MINUTOS</span>`;
                 }
            }
