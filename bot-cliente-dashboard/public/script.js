document.addEventListener('DOMContentLoaded', () => {
    ...
    const showDashboard = () => {
        ...
        const q = (s) => '"' + esc(s) + '"';
        const cols = ['ID','FECHA','EMPRESA','CONTACTO','CATEGORIA','DESCRIPCION','ESTADO','TECNICO','URGENCIA','NOTAS','HISTORIAL'];
        let csv = cols.map(c => q(c)).join(',') + '\n';
        allReports.forEach(r => {
            const f = r.fecha || r['FEHCA Y HORA'] || (r.createdAt ? new Date(r.createdAt).toLocaleString('es-AR') : '');
            const h = (r.history_array||[]).join(' | ');
            csv += [q(r.id),q(f),q(r.empresa||r.Empresa||''),q(r.nombre||r.Nombre||r.contacto||''),q(r.categoria_normalizada||''),q(r.informe||r.Informe||r['DESCRIPCION SUCESO']||''),q(r.estado_normalizado||r.estado||''),q(r.tecnico_normalizado||r.tecnico||'Sin Asignar'),q(r.urgencia||''),q(r.notas_internas||''),q(h)].join(',') + '\n';
        });
        const blob = new Blob([new Uint8Array([0xEF,0xBB,0xBF]),csv],{type:'text/csv;charset=utf-8'});
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = 'QH_' + now.getFullYear() + mm + dd + '.csv'; a.click(); URL.revokeObjectURL(url);
    };
