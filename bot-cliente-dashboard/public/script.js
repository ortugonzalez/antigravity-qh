document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderMain = () => {
        ...
        if (query) {
             finalFiltered = finalFiltered.filter(r => {
                 ...
                 return tEmp.includes(query) || tNom.includes(query) || tId.includes(query);
             });
        }
        ...
        const createItemNode = (req) => {
            ...
            if (urg.includes('alta') || urg.includes('crític') || urg.includes('urgente')) { urgColor = '#ff3366'; borderColor = '#ff3366'; }
            else if (urg.includes('media') || urg.includes('normal')) { urgColor = '#ffcc00'; borderColor = '#ffcc00'; }
            ...
            if (estadoActual.toLowerCase() === 'pendiente' && req.createdAt) {
                 ...
                 if (hoursPassed < 2) {
                     ...
                     borderColor = '#00b4d8';
                     SLA_Html = `<span class="badge" style="background:#00b4d8; color:#fff; border: 1px solid rgba(255,255,255,0.3); font-size: 0.8rem; padding: 5px 10px;">⏱️ < 2Hs</span>`;
                 } else if (hoursPassed < 12) {
                     itemClasses += ' sla-yellow';
                     borderColor = '#ffcc00';
                     SLA_Html = `<span class="badge" style="background:#ffcc00; color:#000; border: 1px solid rgba(0,0,0,0.3); font-size: 0.8rem; padding: 5px 10px;">⚠️ < 12Hs</span>`;
                 } else {
                     itemClasses += ' sla-red';
                     borderColor = '#ff3366';
                     SLA_Html = `<span class="badge" style="background:#ff3366; color:#fff; border: 1px solid rgba(255,255,255,0.4); font-size: 0.8rem; padding: 5px 10px; font-weight: 900;">🔥 > 12Hs (CRÍTICO)</span>`;
                 }
            }
            ...
            const rawClient = req.nombre || req.Nombre || req.NOMBRE || 'Cliente n/a';
            const rawPhone = req.telefono || req.Telefono || req.TELEFONO || '---';
            ...
            item.innerHTML = `
                ...
                <div style="font-size: 0.8rem; color: #a0a0b0; margin-bottom: 5px;">👤 ${rawClient} | 📞 ${rawPhone}</div>
                ...
                <div class="mini-badges">
                    <span class="urgency-dot" style="color:${urgColor}; background-color:${urgColor};"></span>
                    ${SLA_Html}
    ...
    window.openModal = (reportId) => {
        ...
        const rawClient = req.nombre || req.Nombre || req.NOMBRE || 'Cliente n/a';
        const rawPhone = req.telefono || req.Telefono || req.TELEFONO || '---';
        const rawCat = req.categoria_normalizada || req.categoria || req.Categoria || req.CATEGORIA || req['TIPO DE SERVICIO'] || req['Tipo de servicio'] || 'Sin Categoria';
        ...
        mContacto.textContent = `👤 ${rawClient} | 📞 ${rawPhone}`;
