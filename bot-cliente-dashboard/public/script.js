document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderMain = () => {
        ...
        const createItemNode = (req) => {
            ...
            const catDisplay = req.categoria_normalizada || req.categoria || req.Categoria || req.CATEGORIA || req['TIPO DE SERVICIO'] || req['Tipo de servicio'] || req.Tipo || req.tipo || 'Sin Categoria';
            ...
            item.innerHTML = `
                ...
                <div style="font-size: 0.8rem; color: #a0a0b0; margin-bottom: 5px; display:flex; gap:8px; align-items:center;">
                    <span>👤 ${rawClient}</span> 
                    <button class="icon-btn" style="width:24px;height:24px;font-size:0.8rem;" onclick="event.stopPropagation(); navigator.clipboard.writeText('${rawPhone}'); showToast('📱 Teléfono copiado');" title="Copiar Teléfono">📞 ${rawPhone}</button>
                    ${rawEmail ? `<button class="icon-btn" style="width:24px;height:24px;font-size:0.8rem;" onclick="event.stopPropagation(); navigator.clipboard.writeText('${rawEmail}'); showToast('📧 Email copiado');" title="Copiar Email">✉️ ${rawEmail}</button>` : ''}
                </div>
    ...
    window.openModal = (reportId) => {
        ...
        const rawPhone = req['Número'] || req.Número || req.numero || req.Numero || req['NUMERO DE CONTACTO'] || req.telefono || req.Telefono || req.TELEFONO || '---';
        const rawEmail = req['E-mail'] || req.email || req.Email || req.EMAIL || '';
        const rawCat = req.categoria_normalizada || req.categoria || req.Categoria || req.CATEGORIA || req['TIPO DE SERVICIO'] || req['Tipo de servicio'] || req.Tipo || req.tipo || 'Sin Categoria';
        ...
        mContacto.innerHTML = `👤 ${rawClient} | <span style="cursor:pointer; text-decoration:underline; color:#00b4d8;" onclick="navigator.clipboard.writeText('${rawPhone}'); showToast('📱 Teléfono copiado');" title="Copiar Teléfono">📞 ${rawPhone}</span> ${rawEmail ? `| <span style="cursor:pointer; text-decoration:underline; color:#00b4d8;" onclick="navigator.clipboard.writeText('${rawEmail}'); showToast('📧 Email copiado');" title="Copiar Email">✉️ ${rawEmail}</span>` : ''}`;
