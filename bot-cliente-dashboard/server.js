...
app.post('/webhook/n8n-data', (req, res) => {
    ...
    incoming.forEach(item => {
        ...
        if (!existe) {
            ...
            // V6: Respetar log de auditoría si viene de Sheets (opcional), sino crearlo vacio
            data.ultima_actualizacion_log = data.ultima_actualizacion_log || data['Log Auditoria'] || '';
            ...
            if (data.row_number) {
                 ...
                 if(index >= 0) {
                     ...
                     const logLocal = repLocal.ultima_actualizacion_log; // V6: Conservar rastro de UI local
                     ...
                     data.ultima_actualizacion_log = logLocal || data.ultima_actualizacion_log || data['Log Auditoria'] || '';
...
// V6: Auditoria parameters añadidos
app.post('/api/reassign', checkAuth, (req, res) => {
    ...
    if (report) {
        ...
        if (auditLog) report.ultima_actualizacion_log = auditLog;
...
app.post('/api/update-ticket', checkAuth, async (req, res) => {
    ...
    if (auditLog !== undefined) report.ultima_actualizacion_log = auditLog;
        ...
        try {
            await fetch(N8N_SYNC_WEBHOOK, {
                ...
                body: JSON.stringify({
                    ...
                    log_auditoria: report.ultima_actualizacion_log || '' // V6: Enviar el log de vuelta a Sheets si quiere
                })
            });
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 Servidor V6 iniciado en http://0.0.0.0:${PORT}`);
    process.on('SIGTERM', () => {
