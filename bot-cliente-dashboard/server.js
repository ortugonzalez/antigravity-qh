...
app.post('/webhook/n8n-data', (req, res) => {
    ...
    incoming.forEach(item => {
        ...
        // Evitar duplicados: Unimos todos los valores en un string e ignoramos mayúsculas/minúsculas
        const valuesString = Object.values(data).filter(v => typeof v === 'string' && v.trim() !== '').join(' | ').toLowerCase();
        
        const existe = currentReports.find(r => {
            const rValues = Object.values(r).filter(v => typeof v === 'string' && v.trim() !== '' && v !== r.id).join(' | ').toLowerCase();
            return valuesString === rValues;
        });
