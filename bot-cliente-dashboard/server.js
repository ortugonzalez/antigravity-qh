...
app.post('/webhook/n8n-data', (req, res) => {
    ...
    incoming.forEach(item => {
        ...
        const getTecField = (r) => {
            const k = Object.keys(r).find(x => /tecnic|técnico|asign|respons/i.test(x));
            return k && typeof r[k] === 'string' ? r[k] : '';
        };
        ...
        if (!existe) {
            ...
            if (data.row_number) {
                 ...
                 if(index >= 0) {
                     ...
                     if(tecKeyValUpdate) { data.tecnico_normalizado = cleanTecnico(tecKeyValUpdate); }
